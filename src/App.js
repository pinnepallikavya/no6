...
const App = () => {
  ...
  const [imageUrl, setImageUrl] = useState(imagesList[0].imageUrl);
  ...
  const handleThumbnailClick = category => {
    if (category === activeTab) {
      setScore(score + 1);
      setImageUrl(imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl);
    } else {
      setShowScorecard(true);
    }
  };
  ...
  const handlePlayAgain = () => {
    setScore(0);
    setTimeLeft(60);
    setImageUrl(imagesList[0].imageUrl);
  ...
  return (
    <div className="app-container">
      ...
      <ul className="nav-list">
        {tabsList.map(tab => (
          <li key={tab.tabId} className={`nav-item ${activeTab === tab.tabId ? 'active' : ''}`}>
            <button type="button" onClick={() => handleTabClick(tab.tabId)}>
              {tab.displayText}
            </button>
          </li>
        ))}
      ...
      <ul className="thumbnails-list">
        {imagesList
          ...
          .map(filteredImage => (
            <li key={filteredImage.id} className="thumbnail-item">
              <button type="button" onClick={() => handleThumbnailClick(filteredImage.category)}>
                <img src={filteredImage.thumbnailUrl} alt="thumbnail" />
              </button>
            </li>
          ))}
      </ul>
      ...
      {showScorecard && (
        <div className="scorecard-container">
          ...
          <p>YOUR SCORE</p>
          <p>{score}</p>
          <img src="https://assets.ccbp.in/frontend/react-js/match-game-reset-img.png" alt="reset" />
          <button type="button" onClick={handlePlayAgain}>
            PLAY AGAIN
          </button>
        </div>
